...
class Arbol(object):
    ...
    def conta_Diplodocus(self, dic):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.conta_Diplodocus(dic)
            #! chequear que no sea vacio
            if(self.datos["nombre"] in dic):
                dic[self.datos['nombre']] += 1
            else:
                dic[self.datos['nombre']] = 1
            print(self.info, self.datos)
            if(self.der is not None):
                self.der.conta_Diplodocus(dic)


    def separarHyV(self, arbol_villanos, arbol_heroes):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.separarHyV(arbol_villanos, arbol_heroes)
            if(self.der is not None):
                self.der.separarHyV(arbol_villanos, arbol_heroes)
            if(self.datos['heroe']== False):
                arbol_heroes= arbol_heroes.insertar_nodo(self.info, self.datos)
            else:
    ...
    def añade_captura(self):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.añade_captura()
    ...
    def inorden_criaturas_no_derrotadas(self,clave = None):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.inorden_criaturas_no_derrotadas()  
            if (self.datos["derrotado_por"] == ""):  
                print(self.info)
            if(self.der is not None):
                self.der.inorden_criaturas_no_derrotadas()   

    def inorden_capturados_por_heracles(self,clave = None):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.inorden_capturados_por_heracles()  
            if (self.datos["capturada_por"] == "Heracles"):  
                print(self.info)
            if(self.der is not None):
                self.der.inorden_capturados_por_heracles()  
# arbol = Arbol()
